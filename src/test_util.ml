...
let test_negative test_no  =
  Test.make_simple_test
    ~title:("testing cyclic ordering " ^ (string_of_int test_no) ^ " (expecting failure)")
    (fun () ->
      Assert.raises
