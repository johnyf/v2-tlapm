...
let open_tag i tg = let signal = (input i) in match signal  with
  ...
  | `El_start tag -> failwith ("Illegal XML start tag: " ^ (snd(fst tag)) ^
                                 ". expected: " ^ tg ^ ".")
  | `El_end -> failwith ("Illegal tag encountered: expecting start tag for: " ^
                           tg ^ ", but got end tag.")
  | _ -> failwith ("Illegal XML element, expecting open tag: " ^
                     (formatSignal signal))

let close_tag i tg = match (input i) with
  | `El_end -> () (* can we check that we expect the end to be </tg>? *)
  | `El_start d ->
     failwith ("Illegal tag encountered: expecting end tag for: " ^ tg ^
                 ", but got start tag for: " ^ (snd(fst d)))
  | _ ->
     failwith ("Illegal XML element, expecting close tag: " ^
                 (formatSignal (peek i)))
 ...
 * MRI: that only works if the referenced item has been parsed first:
        current plan is parse with ref, then insert refs
 * We need to remember to use mk functions for term sharing
 * TODO in SANY, move the context element to be appended first in the tree
        (before, location, name, etc.)
 * TODO: After resolving refs, the data structure may be cyclic.
...
let get_optchild_choice ?context:(con=None) i tgs_funs =
  ...
  | (_x1::_x2::_xs) as l ->
     failwith ("We expected exactly one matching child in " ^
                 (formatSignal (peek i)) ^ " but got " ^
...
(** processes children (left to right) of the current node. return a list of
    children having the name tg and maps the function f on each of them.
    processing stops after the first child not named tg.
*)
let get_children ?context:(con=None) i tg f =
  get_children_choice ~context:con i [((=) tg,f)]
...
(* expects a node named tg_par and returns the all the children with tag
   tg_chdren.
