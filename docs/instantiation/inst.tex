...
\begin{lemma}
  Let $\sigma$ be an idempotent substitution. Then $dom(\sigma) \cap FV(rg(\sigma))
  = \emptyset$.
\end{lemma}
\begin{proof}
  For the sake of contradiction suppose that there exists $x \in dom(\sigma)
  \cap FV(rg(\sigma))$. Unfolding the definitions of domain and range, we know that
  $x\sigma = s$ form some term $s \neq x$ and that there exists a
  $z \in dom(\sigma)$ and a term $t[x]$ such that $\metavar{z}\sigma =
  t[x]$\footnote{Although $t[x]$ may be $x$ itself.}.

 We distinguish the following shapes of
  $z\sigma$:
  \begin{itemize}
  \item Constant, Variable: This case is not applicable because $t[x]$ must
    contain the formal parameter $x$.
  \item Formal Parameter: Assume $\metavar{z}\sigma=x$. Because
    $x\sigma \neq x$, $\metavar{z}$ must be a different parameter than $x$.
    But then $\metavar{z}\sigma=t[x]$ is different from $\metavar{z}(\sigma
    \fpscat\sigma) = t[s]$ which contradicts the  idempotence of $\sigma$.
  \item Abstraction : Assume  $\metavar{z}\sigma=\lambda u . r[x]$ (with
    $u \neq x$). But $\lambda u . r[x]$ is different from
    $\lambda u \lambda u . t[x]$ which contradicts the idempotence of $\sigma$.
  \item Application: Assume $\metavar{z}\sigma=f(r[x])$ for some $f$. Again,
    $f(t[x])$ is different from $f(t[f(t[x])])$ which contradicts the
    idempotence of $\sigma$.
  \item FP Substitution: Assume $\metavar{z}\sigma=r\tau$ for some term $r$. By
    the definition of the set of free variables, $FV(r\tau)=\set{FV(u\tau) |
      u \in FV(r)}$, so either $r\tau$

%Then $x\sigma$ can  be non-trivial (i.e. different from $x$) because $x \in s \land
%    x\tau \neq x$ or because $x \in s$:
%     \begin{itemize}
%     \item By induction on the sum $n$ of explicit substitution and
%       instantiation nodes: if $n=0$ the problematic cases~\ref{l:fv1}
%       and~\ref{l:fv2} are not applicable. Otherwise we assume as IH that
%       $\tau$
%     \end{itemize}
    \todo[inline]{But $\tau = \tau \fpscat \tau$ so this one is idempotent.}
  \item\label{l:fv2} Instantiation: \todo[inline]{Finish}
  \end{itemize}
